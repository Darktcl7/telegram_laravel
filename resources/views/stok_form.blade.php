...
@section('content')
    ...
    @if(isset($selectedUser))
        <form method="POST" action="{{ route('stok.store') }}" id="stok-form">
            @csrf
            <input type="hidden" name="promotor_id" value="{{ $selectedUser->id }}">
            @if(isset($selectedToko))
                <input type="hidden" name="toko_id" value="{{ $selectedToko->id }}">
            @endif

            <div class="mb-3">
                <label class="form-label">Apakah ada barang baru masuk hari ini?</label>
                <select name="status" id="status" class="form-select" required>
                    <option value="">-- Pilih --</option>
                    <option value="tidak">Tidak Ada Barang Baru</option>
                    <option value="ada">Ada Barang Baru</option>
                </select>
                @error('status') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
            </div>

            {{-- Produk wrapper: will contain one or more produk-block --}}
            <div id="produk-area" style="display:none;">
                <div id="produk-wrapper">
                    {{-- Initial product block (index 0) --}}
                    <div class="produk-block border rounded p-3 mb-3" data-index="0">
                        <div class="produk-header mb-2">
                            <div>
                                <span class="produk-index">#1</span>
                                <strong>Barang</strong>
                            </div>
                            <div>
                                {{-- remove button hidden for first --}}
                                <button type="button" class="btn btn-sm btn-outline-danger remove-produk-btn" style="display:none;">
                        ...
                        <div class="mb-2">
                            ...
                            <select name="produk[0][produk_id]" class="form-select produk-select" required>
                                <option value="">-- Pilih Produk --</option>
                                @foreach($produks as $pr)
                                    <option value="{{ $pr->id }}">{{ $pr->nama_model }} - {{ $pr->varian_ram_rom }} - {{ $pr->warna }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-2">
                            ...
                            <div class="imei-repeater">
                                <div class="input-group mb-2 imei-row">
                                    <input type="text" name="produk[0][imei][]" class="form-control imei-input" placeholder="Scan IMEI..." readonly>
                                ...
                                @error('produk.0.imei.*') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    <button type="button" class="btn btn-success" id="add-produk-btn"><i class="fas fa-plus"></i> Tambah Barang Baru</button>
                </div>
            </div>
        ...
        {{-- scanner overlay --}}
        <div id="scanner-overlay" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.85); z-index: 9999; display: flex; justify-content: center; align-items: center; flex-direction: column;">
            <div id="reader" style="width: 90vw; max-height: 70vh; border: 2px solid white; border-radius: 8px;"></div>
            <button type="button" class="btn btn-danger mt-3" id="close-scanner-btn">Tutup</button>
        </div>
